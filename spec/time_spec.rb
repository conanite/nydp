...
describe Nydp::Date do
  ...
  it "creates a new Time instance" do
    nd = timef.invoke vm, Nydp::Pair.from_list([2015, 11, 18, 18, 30, 17])
    # nd = vm.args.pop
    expect(nd).to be_a Time
  ...
  it "creates a new Time instance for a given Date" do
    nd = timef.invoke_2 vm, Nydp::Date.new(Date.parse "2006-06-21")
    # nd = vm.args.pop
    expect(nd).to be_a Time
